...
export function ComparisonButtons({
  ...
  return (
    <>
      ...
      <div className={inline ? "flex items-center gap-2" : "mt-4 space-y-3"}>
        {showLabelInput ? (
          ...
          <button
            ...
            className={`${inline ? 'px-3 py-2' : 'w-full py-2.5'} rounded-lg text-sm font-medium transition flex items-center justify-center gap-2 ${
              disabled || isFull
          ...
          >
            ...
            {isFull ? `Max ${MAX_COMPARISONS}` : (inline ? 'Compare' : 'Save to Compare')}
          </button>
        )}
        ...
        <button
          ...
          className={`${inline ? 'px-3 py-2' : 'w-full py-2.5'} rounded-lg text-sm font-medium bg-zinc-800 text-zinc-300 hover:bg-zinc-700 border border-zinc-700 transition flex items-center justify-center gap-2`}
        >
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
          </svg>
          {inline ? (count > 0 ? `(${count})` : '') : `View Comparisons ${count > 0 ? `(${count})` : ''}`}
        </button>
      </div>
