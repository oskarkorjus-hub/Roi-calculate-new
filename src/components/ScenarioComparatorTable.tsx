...
const CALCULATOR_METRICS: Record<string, MetricConfig[]> = {
  'mortgage': [
    ...
    { key: 'totalCost', label: 'Total Cost', format: (v) => formatCurrency(v || 0), higher: false },
  ...
  'rental-roi': [
    ...
    { key: 'occupancyRate', label: 'Occupancy', format: (v) => `${(v || 0).toFixed(0)}%`, higher: true },
  ...
  'xirr': [
    ...
    { key: 'totalReturn', label: 'Exit Value', format: (v) => formatCurrency(v || 0), higher: true },
    { key: 'avgCashFlow', label: 'Monthly Rental', format: (v) => formatCurrency(v || 0), higher: true },
  ...
  'dev-feasibility': [
    ...
    { key: 'avgCashFlow', label: 'Net Profit', format: (v) => formatCurrency(v || 0), higher: true },
    { key: 'totalInvestment', label: 'Development Cost', format: (v) => formatCurrency(v || 0), higher: false },
  ],
  'indonesia-tax': [
    { key: 'roi', label: 'Net ROI', format: (v) => `${(v || 0).toFixed(1)}%`, higher: true },
    { key: 'netProfit', label: 'Net Profit', format: (v) => formatCurrency(v || 0), higher: true },
    { key: 'taxSavings', label: 'Depreciation Savings', format: (v) => formatCurrency(v || 0), higher: true },
    { key: 'capitalGainsTax', label: 'Capital Gains Tax', format: (v) => formatCurrency(v || 0), higher: false },
    { key: 'effectiveTaxRate', label: 'Effective Tax Rate', format: (v) => `${(v || 0).toFixed(2)}%`, higher: false },
    { key: 'totalInvestment', label: 'Purchase Price', format: (v) => formatCurrency(v || 0), higher: false },
  ],
  'rental-projection': [
    { key: 'avgCashFlow', label: 'Annual Net Income', format: (v) => formatCurrency(v || 0), higher: true },
    { key: 'annualRevenue', label: 'Gross Revenue', format: (v) => formatCurrency(v || 0), higher: true },
    { key: 'occupancyRate', label: 'Avg Occupancy', format: (v) => `${(v || 0).toFixed(0)}%`, higher: true },
    { key: 'averageNightlyRate', label: 'Nightly Rate', format: (v) => formatCurrency(v || 0), higher: true },
  ],
  'financing': [
    { key: 'monthlyPayment', label: 'Monthly Payment', format: (v) => formatCurrency(v || 0), higher: false },
    { key: 'totalInterest', label: 'Total Interest', format: (v) => formatCurrency(v || 0), higher: false },
  ...
  'dev-budget': [
    ...
    { key: 'landCost', label: 'Land Cost', format: (v) => formatCurrency(v || 0), higher: false },
    { key: 'constructionCost', label: 'Construction', format: (v) => formatCurrency(v || 0), higher: false },
    { key: 'softCosts', label: 'Soft Costs', format: (v) => formatCurrency(v || 0), higher: false },
    ...
    { key: 'contingencyRemaining', label: 'Contingency Left', format: (v) => formatCurrency(v || 0), higher: true },
  ...
  'risk-assessment': [
    ...
    { key: 'annualCashFlow', label: 'Annual Cash Flow', format: (v) => formatCurrency(v || 0), higher: true },
    { key: 'dscr', label: 'DSCR', format: (v) => `${(v || 0).toFixed(2)}x`, higher: true },
    { key: 'leverageRatio', label: 'Leverage', format: (v) => `${((v || 0) * 100).toFixed(0)}%`, higher: false },
