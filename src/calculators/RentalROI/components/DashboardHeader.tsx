...
function MiniTooltip({ text }: { text: string }) {
  ...
  return (
    <div className="relative inline-flex">
      <button
        type="button"
        className="w-3.5 h-3.5 rounded-full bg-zinc-700 hover:bg-zinc-600 text-zinc-400 hover:text-zinc-300 text-[8px] font-bold flex items-center justify-center transition-colors cursor-help ml-1"
      ...
      {show && (
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1.5 bg-zinc-700 text-white text-[10px] rounded-lg shadow-lg whitespace-normal w-48 z-50 normal-case tracking-normal font-normal">
          {text}
      ...
      <div className="bg-zinc-900 p-4 rounded-2xl border border-zinc-800">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 bg-emerald-600 text-white px-3 py-2 rounded-lg text-xs font-bold hover:bg-emerald-700 transition-colors"
              >
                Save
              </button>
              <button
                onClick={() => { setShowLabelInput(false); setSaveLabel(''); }}
                className="px-3 py-2 rounded-lg text-xs font-bold text-zinc-400 hover:bg-zinc-800 transition-colors"
              >
                Cancel
              </button>
            </div>
        ...
        {comparisonCount > 0 && (
          <button
            ...
            className="w-full mt-3 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold bg-zinc-800 border border-zinc-700 text-emerald-400 hover:bg-zinc-700 transition-all"
          >
...
const Card: React.FC<{ title: string; value: string; label: string; tooltip?: string; icon: React.ReactNode; color: string; bg: string; border: string }> = ({ title, value, label, tooltip, icon, color, bg, border }) => (
  <div className={`bg-zinc-900 p-5 rounded-2xl border ${border} transition-all duration-300 hover:border-zinc-600 group cursor-default`}>
